...
Provider.prototype.config = {
     ...
     type: Provider.TabType, // can be one of (MOVIE, TVSHOW, ANIME), see
                             // type documentation for more info.
...
**** type
the type element should be one of Provider.TabType, currently:

#+BEGIN_SRC javascript
    Provider.TabType.MOVIE,   // provider has elements of type 'Provider.ItemType.MOVIE'
    Provider.TabType.TVSHOW,  // provider has elements of type 'Provider.Itemtype.TVSHOW'
    Provider.TabType.ANIME,   // will show a mix between:
